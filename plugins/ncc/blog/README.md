...
The plugin uses the markdown markup for the posts. You can use any Markdown syntax and some special tags for embedding images and videos (requires Ncc Blog Video plugin). To embed an image use the image placeholder:
...
The post list and the pagination are coded in the default component partial `plugins/ncc/blog/components/posts/default.htm`. If the default markup is not suitable for your website, feel free to copy it from the default partial and replace the `{% component %}` call in the example above with the partial contents.
...
The post details is coded in the default component partial `plugins/ncc/blog/components/post/default.htm`.
...
The category list is coded in the default component partial `plugins/ncc/blog/components/categories/default.htm`.
