...
class Session extends ComponentBase
{
    ...
    {
        return [
            'name'        => 'ncc.user::lang.session.session',
            'description' => 'ncc.user::lang.session.session_desc'
        ];
    }
    ...
    public function defineProperties()
    {
        return [
            'security' => [
                'title'       => 'ncc.user::lang.session.security_title',
                'description' => 'ncc.user::lang.session.security_desc',
                'type'        => 'dropdown',
                'default'     => 'all',
                'options'     => [
                    'all'   => 'ncc.user::lang.session.all',
                    'user'  => 'ncc.user::lang.session.users',
                    'guest' => 'ncc.user::lang.session.guests'
                ]
            ],
            'allowedUserGroups' => [
                'title'       => 'ncc.user::lang.session.allowed_groups_title',
                'description' => 'ncc.user::lang.session.allowed_groups_description',
                'placeholder' => '*',
                'type'        => 'set',
                'default'     => []
            ],
            'redirect' => [
                'title'       => 'ncc.user::lang.session.redirect_title',
                'description' => 'ncc.user::lang.session.redirect_desc',
                'type'        => 'dropdown',
                'default'     => ''
            ]
        ];
    ...
    public function onLogout()
    {
        ...
        if ($user) {
            Event::fire('ncc.user.logout', [$user]);
        }

        $url = post('redirect', Request::fullUrl());

        Flash::success(Lang::get('ncc.user::lang.session.logout'));
    ...
    public function onStopImpersonating()
    {
        ...
        Flash::success(Lang::get('ncc.user::lang.session.stop_impersonate_success'));
