...
class User extends UserBase
{
     ...
     */
    public $rules = [
        'email'                 => 'required|between:6,30|email|unique:users',
        'avatar'                => 'nullable|image|max:4000',
        'username'              => 'required|between:2,191|unique:users',
        'name'                  => 'required|between:2,30',
        'password'              => 'required:create|min:8|max:30',
        'password_confirmation' => 'required_with:password|min:8|max:30|same:password',
    ];
    ...
    public $messages = [
        'email.between' => 'The email must be between 6 and 30 characters',
        'email.required' => 'The email field is required',
        'email.unique' => 'The email has already been taken',
        'name.between' => 'The name must be between 6 and 30 characters',
        ...
        'password_confirmation.max' => 'The password confirmation may not be greater than 30 characters',
    ...
    public function attemptActivation($code)
    {
        ...
        Event::fire('ncc.user.activate', [$this]);
    ...
    {
        return Config::get('ncc.user::minPasswordLength', 8);
    }
    ...
    public function afterLogin()
    {
        ...
        if ($this->trashed()) {
            ...
            Mail::sendTo($this, 'ncc.user::mail.reactivate', [
                'name' => $this->name
            ]);

            Event::fire('ncc.user.reactivate', [$this]);
        }
        else {
            parent::afterLogin();
        }

        Event::fire('ncc.user.login', [$this]);
    }
    ...
    public function afterDelete()
    {
        if ($this->isSoftDelete()) {
            Event::fire('ncc.user.deactivate', [$this]);
            return;
        }
    ...
    public function getNotificationVars()
    {
         ...
         * Extensibility
         */
        $result = Event::fire('ncc.user.getNotificationVars', [$this]);
    ...
    /**
     * Sends an invitation to the user using template "ncc.user::mail.invite".
     * @return void
     */
    protected function sendInvitation()
    {
        Mail::sendTo($this, 'ncc.user::mail.invite', $this->getNotificationVars());
    }
